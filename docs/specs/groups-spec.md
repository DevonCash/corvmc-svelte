...
fires correctly), and the `includeBandEvents` boolean whose filter vanished when true. A third — that
...
- Event creation, ticketing, and the public gig guide, beyond the ownership column and the new `source` value.
