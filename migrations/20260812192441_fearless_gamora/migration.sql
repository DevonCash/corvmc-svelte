...
--   ticket, event_rsvp, volunteer_hour_log, volunteer_shift_feedback, volunteer_signup, volunteer_shift
...
CREATE TABLE `__new_event` (
	...
	CONSTRAINT "event_cmc_needs_end" CHECK(source = 'band' OR ends_at IS NOT NULL)
);
--> statement-breakpoint
...
--> statement-breakpoint
-- Backfill the bill from the owner pointer. Every existing band event has its
-- band in `event.band_id`; band-scoped reads now go through `event_band`, so
-- without this every band's gigs would vanish from its own profile.
INSERT INTO event_band (id, event_id, name, band_id, billing_order, status, added_by_band_id, created_at)
SELECT lower(hex(randomblob(16))), e.id, b.name, e.band_id, 0, 'confirmed', e.band_id, unixepoch()
FROM event e JOIN band b ON b.id = e.band_id
