...
/**
 * Owner-typed input reduced to its canonical form. Deliberately the same
 * `generateSlug` the rest of the app uses, so there is one normalization rule.
 */
...
export async function resolveBandSlug(slug: string): Promise<SlugResolution> {
	...
	const [moved] = await db
		...
		.innerJoin(band, eq(band.id, bandSlugHistory.bandId))
		.where(and(eq(bandSlugHistory.slug, slug), isNull(band.deletedAt)))
		.limit(1);
...
export async function changeBandSlug(
	...
	const [row] = await db
		.select({ id: band.id, slug: band.slug, customDomain: band.customDomain })
		.from(band)
	...
	const [taken] = await db
		.select({ id: band.id })
		.from(band)
		.where(and(eq(band.slug, next), ne(band.id, bandId)))
		.limit(1);
	...
	try {
		...
		await db.batch([
			...
			db.update(band).set({ slug: next, updatedAt: new Date() }).where(eq(band.id, bandId))
		]);
	} catch (err) {
