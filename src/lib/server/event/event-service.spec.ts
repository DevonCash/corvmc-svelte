...
describe('EventService', () => {
	...
	// -----------------------------------------------------------------------
	// Band event ticketing
	//
	// A band event is a row in the same table with source='band', so it reaches
	// the same purchase page as a CMC event. These assert the band-facing
	// writers hold the same invariants as the staff ones.
	// -----------------------------------------------------------------------

	describe('createBandEvent ticketing', () => {
		const baseParams = {
			bandId: 'band-1',
		...
		it('stores price and capacity when ticketing is enabled', async () => {
		...
		it('treats a missing capacity as unlimited', async () => {
			await createBandEvent({ ...baseParams, ticketingEnabled: true, ticketPrice: 1500 });

			expect(lastInsertedValues).toMatchObject({ ticketingEnabled: true, ticketQuantity: null });
		});

		it('defaults to unticketed', async () => {
			await createBandEvent(baseParams);
		...
		it('drops a price supplied without the ticketing flag', async () => {
			// Otherwise an event that reads as unticketed still carries a price,
			// which is the state the purchase page would charge against if the
			// flag were ever flipped on elsewhere.
			await createBandEvent({ ...baseParams, ticketPrice: 1500, ticketQuantity: 60 });
		...
		it('rejects enabling ticketing without a price', async () => {
			await expect(createBandEvent({ ...baseParams, ticketingEnabled: true })).rejects.toThrow(
				'Ticket price is required'
			);
		});
		...
		it('rejects a non-positive price', async () => {
			await expect(
				createBandEvent({ ...baseParams, ticketingEnabled: true, ticketPrice: 0 })
			).rejects.toThrow('Ticket price is required');
		});

		it('rejects a fractional-cent price', async () => {
			await expect(
				createBandEvent({ ...baseParams, ticketingEnabled: true, ticketPrice: 1500.5 })
			).rejects.toThrow('Ticket price must be a positive amount');
		});
	});

	describe('updateBandEvent ticketing', () => {
		const existingBandEvent = {
			...mockEventRow,
			status: 'draft',
			bandId: 'band-1',
			source: 'band'
		...
		it('enables ticketing with price and capacity', async () => {
			selectResult = [existingBandEvent];

			await updateBandEvent('evt-1', 'band-1', {
				ticketingEnabled: true,
				ticketPrice: 2000,
				ticketQuantity: 40
			});
		...
		it('clears price and capacity when disabling ticketing', async () => {
			selectResult = [
				{ ...existingBandEvent, ticketingEnabled: true, ticketPrice: 2000, ticketQuantity: 40 }
			];

			await updateBandEvent('evt-1', 'band-1', { ticketingEnabled: false });
		...
		it('rejects enabling ticketing without a price', async () => {
			...
			await expect(updateBandEvent('evt-1', 'band-1', { ticketingEnabled: true })).rejects.toThrow(
				'Ticket price is required'
			);
		});
		...
		it('rejects a non-positive price', async () => {
			...
			await expect(
				updateBandEvent('evt-1', 'band-1', { ticketingEnabled: true, ticketPrice: -100 })
			).rejects.toThrow('Ticket price must be a positive amount');
		});

		it('refuses to touch an event belonging to another band', async () => {
			selectResult = [{ ...existingBandEvent, bandId: 'band-2' }];

			await expect(
				updateBandEvent('evt-1', 'band-1', { ticketingEnabled: true, ticketPrice: 2000 })
			).rejects.toThrow('Event does not belong to this band');
		});
	});
});
