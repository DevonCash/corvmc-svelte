 ...
 * unhandled TypeError that kills the page.
 *
 ...
 * as `routes/member/reservations/+page.svelte` does; splitting the component so
 ...
 * this bug in the reservations page did.
 */
		...
		const offenders = files
			.filter((file) => {
				const { js } = compile(readFileSync(file, 'utf8'), {
					...
					runes: !file.includes('/lib/layouts/'),
